...
Недавно @MAH3IOK поделился со мной интересным функциональным
алгоритмом генерации случайной перестановки списка.
Алгоритм примечательным образом схож с алгоритмом сортировки
слиянием (на котором, кстати, основан sort из Data.List
[http://hackage.haskell.org/package/base-4.7.0.1/docs/src/Data-List.html#sort])
...
3) и так до тех пор, пока не остается один список (функция tillSingletonM) -
   он и будет случайной перестановкой
...
Сложность в точности как у merge sort - O(n*log(n)).
...
< ghci> replicateM_ 5 $ randomShuffle [1..5] >>= print
< [2,5,4,3,1]
< [4,3,2,1,5]
< [1,2,5,4,3]
< [4,5,2,1,3]
...
Но как убедиться в том, что каждая из n! перестановок имеет
одну и ту же вероятность выпасть, равную 1/n! ?
...
>   let hist = Map.unionsWith (+) $ map (flip Map.singleton 1) permutations
>       toProb x = fromIntegral x / fromIntegral n
>   mapM_ (\(p,r) -> putStrLn $ show p ++ " " ++ show (toProb r)) $ Map.toList hist

< ghci> calcProb 4 10^6
< [1,2,3,4] 4.1701e-2
< [1,2,4,3] 4.1917e-2
< [1,3,2,4] 4.1909e-2
< [1,3,4,2] 4.1402e-2
...
(см. например [Functional Pearls: Probabilistic Functional Programming in Haskell (Matrin Erwig et al.)])!
...
(как минимум, я не видел её ни в одном Haskell monad tutorial).
...
[http://hackage.haskell.org/package/set-monad-0.1.0.0/docs/src/Data-Set-Monad.html#Set]:
...
< ghci> print $ runDist (randomShuffle [1..4])
< [1,2,3,4]: 1 % 24
< [1,2,4,3]: 1 % 24
< [1,3,2,4]: 1 % 24
< [1,3,4,2]: 1 % 24
...
для ограниченного множества списков (n! растет черезчур быстро),
