...
http://monadmadness.wordpress.com/2014/11/10/purely-functional-graph-search-algorithms/
...
всего O(|V|) памяти, где |V| - количество вершин в графе!

Это немного контринтуитивно, ведь если сложить длины всех |V| путей
получится O(|V|*log2|V|). А ведь это только оценка для полного бинарного
дерева (доказательство оставляю читателю).

Для вырожденого линейного графа оценка будет и вовсе O(|V|^2)!
...
Можно достичь этого с помощью GADTs или nested datatype.
...
Определение 'transpose'' повторяет 'transpose' из Data.List с очевидными измениями.
