...
через список, которая имеет O(n) сложность для операции вставки
в конец - 'enqueue'.

Можно было бы реализовать очередь через два стека - операции
вставки и извлечения из очереди занимали бы амортизированное O(1).
...
< > print $ tracesFirst $ map get $ levelOrderQueueTrace $ binTree 3
< enqueue [1]
< dequeue []
< enqueue [2]
< enqueue [2,3]
...
этой функции за счет value recursion 'queue'.
...
Иначе из очереди извлекается один элемент (-1) и добавляется (length ts) новых
к (n) уже имеющимся.

>     go n (Tree a ts : rest) = ts ++ go (n - 1 + length ts) rest

Такая очередь называется корекурсивной очередью.
Она описана в отличном туториале
http://www.melding-monads.com/files/CorecQueues.pdf, где также предложен
способ оформить такую очередь в typeclass 'MonadQueue' с функциями 'enqueue' и 'dequeue'.
